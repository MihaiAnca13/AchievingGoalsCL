...
from rl_games.common.train_methods import *
from rl_games.common.meta_methods import *
from rl_games.common.icm import ICM
import numpy as np
import time
import gym
...
class A2CBase:
    def __init__(self, base_name, config):
        ...
        self.experiment_dir = os.path.join(self.train_dir, self.experiment_name) + datetime.now().strftime("_%d-%H-%M-%S")
        ...
        train_method = config.get('train_method', None)
        self.train_method = None
        method_cfg = config.get('train_method_params', None)
        if train_method == "curriculum":
            self.train_method = Curriculum(config.get('numProps', 0) - 1, self.vec_env.env, self.writer, config.get('maxProps', 3))
        elif train_method == "staircased":
            self.train_method = StaircasedInterleaving(self.vec_env.env, self.writer, threshold=method_cfg['s_threshold'])
        elif train_method == "gaussian":
            self.train_method = Gaussian(self.vec_env.env, self.writer, speed=method_cfg['g_speed'], mean=method_cfg['g_mean'], std=method_cfg['g_std'], success_weight=method_cfg['g_success_weight'])
        elif train_method == "redistributed":
            self.train_method = Redistributed(self.vec_env.env, self.writer)
        elif train_method == "aco":
            self.train_method = ACO(self.vec_env.env, self.writer, decay=method_cfg['a_decay'], alpha=method_cfg['a_alpha'], beta=method_cfg['a_beta'], pheromone_multiplier=method_cfg['a_pheromone_multiplier'])
        elif train_method is not None:
            exit(f'Unrecognised train method {train_method}')

        meta_method = config.get('meta_method', None)
        self.meta_method = None
        if meta_method == "reptile":
            self.meta_method = Reptile(self.writer)
        elif meta_method == "parallel-reptile":
            self.meta_method = ParallelReptile(self.writer)
        elif meta_method is not None:
            exit(f'Unrecognised meta method {meta_method}')

        self.icm_module = None
        if config.get('icm', None) is True:
            icm_lr = self.config.get('icm_lr', float(1e-3))
            icm_hidden_size = self.config.get('icm_hidden_size', 128)
            self.icm_module = ICM(self.env_info['observation_space'].shape[0], self.env_info['action_space'].shape[0],
                                  icm_hidden_size, icm_lr, self.writer, self.device)
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            with torch.enable_grad():
                if self.icm_module is not None:
                    self.icm_module.update(old_obs, res_dict['actions'], self.obs['obs'], self.frame)
                    rewards = self.icm_module.get_new_reward(old_obs, res_dict['actions'], self.obs['obs'], self.frame)

            self.experience_buffer.update_data('next_obses', n, self.obs['obs'])

            if self.train_method is not None:
                self.train_method.update(self.dones, infos, self.frame)
...
class ContinuousA2CBase(A2CBase):
    ...
    def train(self):
        ...
        while True:
            # TODO: meta method reptile update here
            epoch_num = self.update_epoch()
