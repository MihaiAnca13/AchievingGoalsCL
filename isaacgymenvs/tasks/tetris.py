...
class Tetris(VecTask):
    ...
    def __init__(self, cfg, sim_device, graphics_device_id, headless):
        ...
        self.height_reached_scale = self.cfg["env"]["heightReachedScale"]
        ...
        # prop dimensions
        self.prop_height = 0.03
        self.prop_width = 0.03
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        prop_asset_I = self.gym.load_asset(self.sim, asset_root, "urdf/tetris/I.urdf", box_opts)
        prop_asset_L = self.gym.load_asset(self.sim, asset_root, "urdf/tetris/L.urdf", box_opts)
        prop_asset_O = self.gym.load_asset(self.sim, asset_root, "urdf/tetris/O.urdf", box_opts)
        prop_asset_T = self.gym.load_asset(self.sim, asset_root, "urdf/tetris/T.urdf", box_opts)
        prop_asset_Z = self.gym.load_asset(self.sim, asset_root, "urdf/tetris/Z.urdf", box_opts)
        # prop_assets = [prop_asset_I, prop_asset_L, prop_asset_O, prop_asset_T, prop_asset_Z]
        prop_assets = [prop_asset_T, prop_asset_L, prop_asset_I]
        ...
        num_prop_bodies = self.gym.get_asset_rigid_body_count(prop_asset_I)
        num_prop_shapes = self.gym.get_asset_rigid_shape_count(prop_asset_I)
        ...
        for i in range(self.num_envs):
            ...
            # setting up props
            props_per_row = int(np.ceil(np.sqrt(self.max_props)))
            ...
            for j in range(props_per_row):
                ...
                for k in range(props_per_row):
                    ...
                    prop_handle = self.gym.create_actor(env_ptr, prop_assets[prop_count], prop_state_pose,
    ...
    def compute_reward(self):
        ...
        self.rew_buf[:], self.reset_buf[:], success, self.mask_per_goal = compute_old_franka_reward(
                                                       ...
                                                       self.done_reward_scale, self.action_penalty_scale, self.height_reached_scale,
    ...
    def compute_observations(self):
        ...
        self.obs_buf = torch.cat((dof_pos_scaled, dof_vels * self.dof_vel_scale, hand_pos, obj_pos_vel, goal_pos, prop_id), dim=-1)
        self.states_buf = self.obs_buf.clone()
    ...
    def reset_idx(self, env_ids):
        ...
        # calculate goal
        on_table_height = self.table_height + 0.025
        r1 = torch.tensor([0.35, -0.3, on_table_height], device=self.device)
        r2 = torch.tensor([0.75, 0.3, on_table_height], device=self.device)
        ...
        self.goal[one_prop_env_id, -1] = (0.45 - on_table_height) * torch.rand(len(one_prop_env_id), device=self.device) + on_table_height
        ...
        N = 5
        x = torch.linspace(0.35, 0.75, N, device=self.device)
        y = torch.linspace(-0.3, 0.3, N, device=self.device)
        x_i, y_i = [], []
        for _ in range(len(env_ids)):
            ii, jj = np.unravel_index(np.random.choice(np.arange(N*N), size=self.max_props, replace=False), (N, N))
            x_i.append(ii)
            y_i.append(jj)
        ...
        or_mask = torch.rand((len(env_ids), self.max_props)) < 0.5
        or_mask = or_mask.to(self.device)
        zeros_x = torch.zeros((len(env_ids), self.max_props), device=self.device)
        zeros_y = torch.zeros((len(env_ids), self.max_props), device=self.device)
        zeros_x[or_mask] += np.pi/2
        zeros_y[or_mask] += np.pi
        # new_pos[:, :, 3:7] = torch.transpose(euler_to_quat(zeros_x, zeros_y, torch.rand((len(env_ids), self.max_props), device=self.device)*2*np.pi), 0, 1)
...
def compute_franka_reward(actions, num_envs: int, num_props, max_props: int, prop_order, hand_pos, hand_orn, goal_pos,
                          props_pos, prop_height: float, dist_reward_scale: float, done_reward_scale: float, action_penalty_scale: float, height_reached_scale: float,
    ...
    ordered_achieved_mask_xy = ((ordered_props_pos[:, :, :2] - goal_pos[:, :2].unsqueeze(1)).abs() <= distance_threshold).sum(-1) == 2
    one_prop_mask = num_props == 1
    ordered_achieved_mask_xy[one_prop_mask] = ((ordered_props_pos[one_prop_mask] - goal_pos[one_prop_mask].unsqueeze(1)).abs() <= distance_threshold).sum(-1) == 3

    for i in range(max_props):
        # set to false objects that are not part of the goal
        mask = num_props == (i + 1)
        ordered_achieved_mask_xy[mask, i + 1:] = False
        if i > 0:
            # achieved only when previous objects are placed
            ordered_achieved_mask_xy[:, i] = torch.bitwise_and(ordered_achieved_mask_xy[:, i], ordered_achieved_mask_xy[:, i-1])
            # check height order
            ordered_achieved_mask_xy[:, i] = torch.bitwise_and(ordered_achieved_mask_xy[:, i], ordered_props_pos[:, i, 2] > ordered_props_pos[:, i-1, 2] + 0.01)
    ...
    dist_to_goal = ordered_props_pos - goal_pos.unsqueeze(1)
    ...
    on_target_reward = ordered_achieved_mask_xy.sum(-1)
    adjusted_hand_pos = hand_pos.clone()
    adjusted_hand_pos[:, 2] -= 0.04
    adjusted_hand_pos = adjusted_hand_pos.unsqueeze(-1).repeat(1, 1, max_props).transpose(1, 2)
    away_from_obj = ((adjusted_hand_pos - ordered_props_pos).abs() > distance_threshold).sum(-1) > 0
    away_from_obj = torch.ones_like(away_from_obj, dtype=torch.bool) # disabling away from obj condition
    achieved_per_box_mask = torch.bitwise_and(ordered_achieved_mask_xy * mask_per_goal, away_from_obj)
    achieved_per_box = achieved_per_box_mask.sum(-1)
    mask_per_goal[achieved_per_box_mask] = False

    # bonus based on how high it reached
    new_prop_id = (~mask_per_goal).sum(-1)
    for i in range(max_props):
        mask = num_props == (i + 1)
        new_prop_id[mask] = torch.clamp(new_prop_id[mask], max=i)
    previous_prop_id = new_prop_id - 1
    height_reached = props_pos[torch.arange(num_envs), previous_prop_id, 2]
    # zero if the previous object is no longer placed
    not_placed_mask = ordered_achieved_mask_xy[torch.arange(num_envs), previous_prop_id]
    # zero if it's only the first object
    not_placed_mask[previous_prop_id < 0] = False
    height_reached[~not_placed_mask] = 0

    achieved = torch.zeros_like(num_props, dtype=torch.bool)
    for i in range(max_props):
        mask = num_props == (i + 1)
        if mask.count_nonzero() > 0:
            achieved[mask] = (~mask_per_goal[mask]).sum(-1) == (i + 1)

    final_reward = on_target_reward - dist_to_goal_reward * dist_reward_scale - action_penalty * action_penalty_scale + achieved_per_box * done_reward_scale - to_box_reward * dist_reward_scale + touching_table_reward + orn_err_reward + achieved * done_reward_scale + height_reached * height_reached_scale

    return final_reward, achieved, achieved, mask_per_goal


@torch.jit.script
def compute_old_franka_reward(actions, num_envs: int, num_props, max_props: int, prop_order, hand_pos, hand_orn, goal_pos,
                          props_pos, prop_height: float, dist_reward_scale: float, done_reward_scale: float, action_penalty_scale: float, height_reached_scale: float,
                          distance_threshold: float, table_height: float, touch_table_penalty: float, upright_orn_scale: float, dofs, lower_limit, upper_limit,
                          mask_per_goal):
    goals = goal_pos.unsqueeze(dim=1)
    ...
    touching_table = torch.zeros_like(to_box_reward)
    touching_table_reward = torch.where(hand_pos[:, 2] <= table_height + 0.06, touching_table - touch_table_penalty, touching_table)
    ...
    adjusted_hand_pos[:, 2] -= 0.04
    adjusted_hand_pos = adjusted_hand_pos.unsqueeze(-1).repeat(1, 1, max_props).transpose(1, 2)
    grasping = ((adjusted_hand_pos - ordered_props_pos).abs() <= distance_threshold).sum(-1) == 3
    grasping = grasping[torch.arange(num_envs), unordered_prop_id]
    ...
    final_reward = grasping.int() * 1 + on_target_reward - dist_to_goal_reward * dist_reward_scale - action_penalty * action_penalty_scale + achieved_per_box * done_reward_scale - to_box_reward * dist_reward_scale + touching_table_reward + orn_err_reward + achieved * done_reward_scale
